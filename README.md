...
| `pq-light-client` | A regtest-only LDK Node wallet and channel manager backed by direct Bitcoin peers. It uses the PQLN fork for PQ peer transport, channels, invoice creation, and fail-closed payment routing. An interactive CLI can fund its wallet, connect to a PQ peer, open a channel, create an invoice, and send a payment after checking a separately trusted ML-DSA key. |
...
`pq-light-client` is a **research regtest client**, not a production wallet. It connects directly to two Bitcoin P2P peers and runs no local Bitcoin full node or Esplora service. It requires both peers to agree on BIP157/158 filter commitments, then saves the full validated filter history in `compact-filter-history.sqlite` under the node's storage directory. It matches filters against the wallet's revealed addresses and LDK's registered channel scripts. Unrelated blocks advance as headers; matching blocks are downloaded. On restart, the payment node checks the archived headers and filter bodies, reconciles their branch with the peers' current headers, and downloads filters only after the first missing or replaced block. The archive retains both sides of a reorganization; historical matches are checked against the current branch. A local regtest check reused 106 filter bodies without redownloading them, fetched only two replacement filters after an offline reorganization, caught a tampered peer, found a transaction after its script was registered late, and rejected a corrupted archived filter. Two peers can still collude or share faulty infrastructure. The client cannot recover a wallet or channel from seed words alone, so it is not yet safe for real funds. See [Integration path](docs/integration.md).
...
`ChainWatch` reuses Winnow's Bitcoin P2P library at a pinned revision. This is a source dependency in this repository; no source files or settings are added to Winnow. The payment-capable node uses [Kyoto's BIP157 peer client](https://github.com/2140-dev/kyoto) inside the LDK Node patch, because its LDK chain listeners are in Rust. Kyoto is the only compact-filter scanner in the payment app. `ChainWatch` is an optional, independent read-only research tool; it does not share the payment node's archive or run alongside it. Winnow's `filters.json` saves scan progress and selected filter headers, whereas the Lightning archive saves the complete filter bodies needed for late channel watches.
...
| [Kyoto](https://github.com/2140-dev/kyoto) | `fd0340d8060f4c95bba69b4f60cf7d65a6771c0a` | Direct Bitcoin P2P synchronization and transaction relay |
| [LDK Node](https://github.com/lightningdevkit/ldk-node) | `b812128c51c0171f510d81847b3ed18f0c34a294` | Regtest wallet, channel manager, monitor persistence, and direct peer adapter; adapted by [`patches/ldk-node-pqln.patch`](patches/ldk-node-pqln.patch) |
