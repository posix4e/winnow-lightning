...
fn verify(invoice_text: &str, key_text: &str) -> Result<String, String> {
...
fn run() -> Result<(), String> {
    ...
    if args.len() != 3 {
        return Err("usage: pq-invoice-verify INVOICE_FILE TRUSTED_ML_DSA_KEY_HEX_FILE".into());
    }
    let invoice_text = fs::read_to_string(&args[1]).map_err(|error| error.to_string())?;
    let key_text = fs::read_to_string(&args[2]).map_err(|error| error.to_string())?;
    println!("{}", verify(&invoice_text, &key_text)?);
    Ok(())
}
