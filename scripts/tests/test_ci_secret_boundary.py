...
class AggregateGateTests(unittest.TestCase):
    def test_failed_cancelled_or_unexpectedly_skipped_deployment_blocks_gate(self):
        ...
        for hosted, tdx, website, lightning, deploy in itertools.product(statuses, statuses, statuses, statuses, ('true', 'false')):
            expected = ((hosted, tdx) in [('success', 'skipped'), ('skipped', 'success')]
                        and website == ('success' if deploy == 'true' else 'skipped') and lightning == 'success')
            with self.subTest(hosted=hosted, tdx=tdx, website=website, lightning=lightning, deploy=deploy):
                result = subprocess.run(['bash', '-e', '-c', script],
                                        env={**os.environ, 'HOSTED': hosted, 'TDX': tdx,
                                             'WEBSITE': website, 'LIGHTNING': lightning, 'DEPLOY': deploy},
                                        capture_output=True)
